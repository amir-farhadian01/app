...
import { Shield, CheckCircle2, AlertCircle, FileText, PenTool, ArrowLeft, Loader2, CreditCard, Banknote } from 'lucide-react';
...
export default function ContractView() {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        const c = await api.get<any>(`/api/contracts/${id}`);
        if (cancelled) return;
        setContract(c);
        if (c?.paymentMethod) setPaymentMethod(c.paymentMethod);
      } catch (e) {
        await handleApiError(e, OperationType.GET, `contracts/${id}`);
  ...
  const handleSign = async () => {
    ...
    setIsSigning(true);
    try {
      await api.put(`/api/contracts/${contract.id}/sign`, { paymentMethod });
      const refreshed = await api.get<any>(`/api/contracts/${contract.id}`);
      setContract(refreshed);
    } catch (error) {
      await handleApiError(error, OperationType.UPDATE, `contracts/${contract.id}`);
    } finally {
      setIsSigning(false);
    }
  };
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (contract.requestId) {
        await api.put(`/api/requests/${contract.requestId}/status`, { status: 'completed' });
      }
      navigate('/dashboard');
    } catch (error) {
      await handleApiError(error, OperationType.UPDATE, `contracts/${contract.id}`);
  ...
  if (!contract) {
    return (
      <div className="max-w-md mx-auto text-center py-20 space-y-6">
        ...
        <button onClick={() => navigate('/dashboard')} className="text-sm font-bold text-neutral-400 hover:text-neutral-900 underline">
          Return to Dashboard
        </button>
      </div>
    );
    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20">
      <header className="flex items-center gap-4">
        <button
          ...
          className="p-3 hover:bg-white rounded-2xl transition-all border border-transparent hover:border-neutral-100"
        >
        ...
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-white p-10 rounded-[3rem] border border-neutral-100 shadow-sm space-y-6">
            ...
            <div className="prose prose-neutral max-w-none text-sm leading-relaxed text-neutral-600">
              <p>
                This agreement is entered into between the customer and the service provider for the work described in service
                request {contract.requestId}.
              </p>
              <h4 className="font-black text-neutral-900 mt-6">1. Provider obligations</h4>
              <p>
                The provider agrees to perform the services with reasonable care and by any agreed deadline. Failure to perform
                may make the provider liable as described in platform policies.
              </p>
              <h4 className="font-black text-neutral-900 mt-6">2. Customer obligations</h4>
              <p>
                The customer agrees to pay the agreed amount after final approval of the work. If cash payment is selected, the
                provider is responsible for settling the platform commission.
              </p>
              <h4 className="font-black text-neutral-900 mt-6">3. Platform commission</h4>
              <p>
                The platform collects $
                {Number(contract.commissionAmount || contract.amount * 0.1).toLocaleString()} as a software and marketplace
                fee for facilitating this job.
              </p>
            </div>
          </section>
        ...
        <div className="space-y-8">
          <section className="bg-neutral-900 text-white p-10 rounded-[3rem] space-y-8 shadow-2xl shadow-neutral-900/20">
            <div className="space-y-1">
              ...
              <h3 className="text-5xl font-black italic">${Number(contract.amount).toLocaleString()}</h3>
            </div>
            ...
            <div className="space-y-4 pt-4 border-t border-white/10">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-black text-emerald-400">
                  ${(contract.amount - (contract.commissionAmount || contract.amount * 0.1)).toLocaleString()}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-amber-700/70 leading-relaxed">
                {contract.paymentMethod === 'cash'
                  ...
                  : 'Payment runs through the platform. Funds are held with standard checkout protections.'}
              </p>
            </div>
          </div>
        </div>
