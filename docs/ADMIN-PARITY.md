...
| Order wizard (F5) | Admin Orders panel (list filters + detail drawer: timeline/audit, read-only chat thread via `GET /api/admin/chat/thread/:orderId`, contract versions via `GET /api/admin/contracts?orderId=`, payment placeholder) + **Overview** DB-backed KPIs (`GET /api/admin/stats`, `GET /api/admin/stats/orders-trend`, `GET /api/admin/audit-log`). Sprint N schema extension adds `Order.broadcastList` + `JobRecord`/`JobStatus` for offer-to-job analytics; existing Admin Orders visibility remains sufficient (no new admin screen in this sprint). | ✅ |
...
| Provider web inbox: accept / decline / mark job complete (`POST /api/orders/:id/complete`) + contract-summary hints on inbox payloads (`contractSummary.currentVersionStatus`) + order detail drawer (scope, package/BOM, scheduling) and in-drawer order chat (`GET/POST /api/orders/:id/chat/*`, NATS `chat.message.created` on server) | Admin order list/detail already shows status transitions; no new admin action required for this sprint | ✅ n/a |
| Provider Company dashboard **Finance** tab (read-only snapshot: orders + internal `Transaction` rows; payout placeholder; no payment processor) | Admin **Payments** + order detail already cover platform-side visibility; provider view is operational-only | ✅ n/a |
| Provider Company dashboard **Staff** tab (workspace member list + workspace vs platform role labels; invite/remove deferred until APIs enforce owner/admin) | Admin **Users** / staff directory covers platform roles and assignments; provider view is workspace membership visibility only | ✅ n/a |
| CRM user inspection (profile, workspaces, wizard orders summary, KYC snapshot, audit, legacy activity) | Platform → **Users**: `UserDetailPanel` + `GET /api/admin/users/:id/full`; Admin Orders filter `userId` + dashboard `ordersUserId` deep-link | ✅ |
...
| Contracts (F8) | Code-defined templates + provider picker (`GET /api/orders/:id/contracts/templates`, `POST .../draft-from-template`); DB-authored template CMS deferred. Dispute queue deferred. **Contract queue + overrides** (Platform → **Contracts**, `GET /api/admin/contracts/queue`, detail drawer, `POST .../mark-reviewed` or `.../reviewed`, `POST .../internal-note` or `.../note`, override-supersede) | ✅ Sprint L queue + web admin UI |
