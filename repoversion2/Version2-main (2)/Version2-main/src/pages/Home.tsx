...
export default function Home() {
  ...
  const { getContent, getConfig } = useTheme();
  
  const h1Content = getContent('h1');
  const pContent = getContent('p');
  const spanContent = getContent('span');
  const sectionConfig = getConfig('section'); // Broad section config
  const featuredConfig = getConfig('div'); // Targeted at card containers
  
  const filteredCompanies = featuredCompanies.filter(c => {
    const filters = sectionConfig?.filters;
    if (!filters) return true;
    return Object.entries(filters).every(([k, v]) => String(c[k]) === String(v));
  });

  useEffect(() => {
    const fetchCompanies = async () => {
      const q = query(collection(db, 'companies'), limit(4));
      const snap = await getDocs(q);
      setFeaturedCompanies(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    };
    fetchCompanies();
  }, []);
    ...
    <div className="space-y-24">
        ...
        <div className="relative z-10 text-center space-y-8 max-w-3xl mx-auto">
          ...
          >
            <span className="inline-block px-4 py-1.5 bg-app-card text-neutral-600 dark:text-neutral-400 text-xs font-bold uppercase tracking-widest rounded-full mb-6 border border-app-border shadow-sm">
              {spanContent?.text || "Trusted by 10,000+ Neighbors"}
            </span>
            <h1 className="text-5xl md:text-7xl font-bold tracking-tight text-app-text leading-[1.1]">
              {h1Content?.text?.split('<br />')[0] || "Your neighborhood,"} <br />
              <span className="text-neutral-400 italic font-serif">{h1Content?.text?.split('<br />')[1]?.replace('<span className="text-neutral-400 italic font-serif">', '').replace('</span>', '') || "at your service."}</span>
            </h1>
            <p className="mt-6 text-lg text-neutral-500 max-w-xl mx-auto leading-relaxed">
              {pContent?.text || "Find trusted local professionals for everything from home repairs to personal training. Simple, secure, and right next door."}
            </p>
          </motion.div>
      ...
      <section className="space-y-12">
        ...
        <div className={cn(
          ...
          sectionConfig?.variant === 'brutalist' && "gap-0",
          sectionConfig?.variant === 'minimalist' && "gap-x-12 gap-y-16"
        )}>
          {filteredCompanies.length > 0 ? filteredCompanies.map((company, i) => (
            <motion.div
              ...
              className={cn(
                "group relative h-80 bg-neutral-100 dark:bg-neutral-800 rounded-[2.5rem] overflow-hidden cursor-pointer shadow-xl shadow-neutral-200/20 dark:shadow-none border border-app-border",
                sectionConfig?.variant === 'brutalist' && "rounded-none border-4 border-black dark:border-white",
                sectionConfig?.variant === 'minimalist' && "rounded-sm border-none shadow-none bg-white dark:bg-neutral-900 border-b border-app-border pb-6",
                sectionConfig?.variant === 'glassmorphism' && "bg-white/10 backdrop-blur-xl border-white/20"
              )}
            >
              <img 
