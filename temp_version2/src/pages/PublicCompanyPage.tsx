...
import { Building2, Star, MapPin, Phone, Globe, Instagram, Facebook, Twitter, Linkedin, CheckCircle2, Briefcase, Users, MessageSquare } from 'lucide-react';
...
export default function PublicCompanyPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const companyData = await api.get<any>(`/api/companies/${id}`);
        setCompany(companyData);

        // Fetch services for this provider
        if (companyData.ownerId) {
          const servicesData = await api.get<any[]>(`/api/services?providerId=${companyData.ownerId}`);
