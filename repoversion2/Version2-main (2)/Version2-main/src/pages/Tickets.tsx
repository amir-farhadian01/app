...
import { collection, query, where, onSnapshot, addDoc, updateDoc, doc, arrayUnion, getDocs, getDoc } from 'firebase/firestore';
...
export default function Tickets() {
  ...
  useEffect(() => {
    ...
    // Also listen for tickets where user is the recipient (if provider)
    const q2 = query(
      collection(db, 'tickets'),
    ...
    const unsub1 = onSnapshot(q, (snapshot) => {
      ...
      setTickets(prev => {
        const combined = [...tks, ...prev.filter(p => p.recipientId === user.uid)];
    ...
    const unsub2 = onSnapshot(q2, (snapshot) => {
      const tks = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setTickets(prev => {
        const combined = [...tks, ...prev.filter(p => p.creatorId === user.uid)];
        return Array.from(new Map(combined.map(item => [item.id, item])).values());
      });
    });
    ...
    const fetchProviders = async () => {
      const pSnap = await getDocs(query(collection(db, 'users'), where('role', '==', 'provider')));
      setProviders(pSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    };
    fetchProviders();
  ...
  const handleCreateTicket = async () => {
    ...
    try {
      const ticket = {
        ...
        messages: [{
          senderId: user?.uid,
          text: newTicketData.initialMessage,
          timestamp: new Date().toISOString()
        }],
