...
export function ActiveOrdersStrip({ onLoad }: ActiveOrdersStripProps) {
  ...
  useEffect(() => {
    let cancelled = false
    api.get<{ items: MyOrderListItem[]; total: number }>('/api/orders', {
      params: { pageSize: 3, status: ACTIVE_STATUSES.join(',') },
    })
      .then((r) => {
        if (cancelled) return
        setItems(r.data.items.slice(0, 3))
        onLoad?.(r.data.total)
      })
      .catch(() => {
