  ...
  const getStrategicAdvice = async () => {
    ...
    try {
      const prompt = `
        ...
        Provide a professional strategic advice report in English.
        Include:
      ...
      setAdvice(response.text || "Analysis failed due to a processing error.");
    } catch (err) {
      console.error(err);
      setAdvice("Error communicating with the Intelligence Mentor.");
    } finally {
          ...
          <div className="bg-app-card p-10 rounded-[3rem] border border-app-border shadow-sm space-y-8 min-h-[600px] flex flex-col">
            ...
            <div className="flex-1 prose prose-invert max-w-none">
              {loadingAdvice ? (
                ...
                <div className="text-app-text space-y-6 leading-loose text-lg whitespace-pre-wrap font-medium">
                  {advice}
                </div>
              )}
            </div>
