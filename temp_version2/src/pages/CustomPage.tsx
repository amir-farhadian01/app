...
export default function CustomPage() {
  ...
  useEffect(() => {
    ...
    const fetchPage = async () => {
      try {
        const data = await api.get<any>(`/api/system/pages/${slug}`);
        setPage(data);
      } catch {
        setPage(null);
      } finally {
  ...
  if (!page || page.status !== 'published') {
    return (
      <div className="max-w-2xl mx-auto py-20 px-6 text-center space-y-6">
        ...
        <button
          onClick={() => navigate('/')}
          className="px-8 py-4 bg-neutral-900 text-white rounded-2xl font-bold text-sm"
        >
          Back to Home
        </button>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-neutral-400">
            <div className="flex items-center gap-1">
              ...
              Last updated {new Date(page.lastEdit || page.updatedAt).toLocaleDateString()}
            </div>
          </div>
        </div>
      </header>
