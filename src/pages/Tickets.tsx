...
import { MessageSquare, Send, Plus, Search, Filter, Clock, CheckCircle2, AlertCircle, User, Shield, Briefcase, ChevronRight } from 'lucide-react';
...
export default function Tickets() {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        const list = await api.get<any[]>('/api/tickets');
        if (cancelled) return;
        setTickets(list || []);
        setLoading(false);
      } catch (e) {
        await handleApiError(e, OperationType.LIST, 'tickets');
      }
    };

    const loadProviders = async () => {
      try {
        const services = await api.get<any[]>('/api/services?limit=200');
        const map = new Map<string, any>();
        (services || []).forEach((s) => {
          if (s.providerId && s.provider) map.set(s.providerId, { id: s.providerId, displayName: s.provider.displayName });
        });
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const updated = await api.put<any>(`/api/tickets/${selectedTicket.id}/message`, { text: newMessage });
      setSelectedTicket(updated);
      setTickets((prev) => prev.map((t) => (t.id === updated.id ? updated : t)));
      setNewMessage('');
    } catch (error) {
      await handleApiError(error, OperationType.UPDATE, 'tickets');
    }
  };
  ...
  const handleCreateTicket = async () => {
    ...
    try {
      const recipientId =
        newTicketData.type === 'client_to_admin' ? undefined : newTicketData.recipientId || undefined;
      const created = await api.post<any>('/api/tickets', {
      ...
      await handleApiError(error, OperationType.CREATE, 'tickets');
    }
  };
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <button 
                    ...
                    className={cn(
                      "p-4 rounded-2xl border transition-all text-left space-y-2",
                      newTicketData.type === 'client_to_provider' ? "bg-neutral-900 border-neutral-900 text-white" : "bg-app-input border-app-border"
                    )}
                  >
                ...
                {newTicketData.type === 'client_to_provider' && (
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400 ml-2">Select Provider</label>
                    <select 
                      value={newTicketData.recipientId}
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    className="w-full p-4 bg-app-input border border-app-border rounded-2xl focus:outline-none focus:ring-2 focus:ring-app-text transition-all resize-none text-app-text"
                  />
                </div>
              </div>
