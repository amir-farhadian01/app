...
export default function PublicCompanyPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, 'companies', id), async (docSnap) => {
      if (docSnap.exists()) {
        const data = { id: docSnap.id, ...docSnap.data() } as any;
        setCompany(data);
        
        // Fetch services for this company/owner
        const sQ = query(collection(db, 'services'), where('providerId', '==', data.ownerId));
        const sSnap = await getDocs(sQ);
        setServices(sSnap.docs.map(s => ({ id: s.id, ...s.data() })));
      }
      setLoading(false);
    }, (error) => handleFirestoreError(error, OperationType.GET, `companies/${id}`));
