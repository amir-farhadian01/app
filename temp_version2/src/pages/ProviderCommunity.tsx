...
import { MessageSquare, Users, Search, Send, Hash, Info } from 'lucide-react';
...
export default function ProviderCommunity() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => fetchMessages(activeRoom.id), 10000);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const msg = await api.post<any>(`/api/chat/rooms/${activeRoom.id}/messages`, {
        text: newMessage,
      });
  ...
  const handleUpdateMessage = async (_messageId: string, _newText: string) => {
    // Not in current API spec — no-op
  };
  ...
  const joinRoom = async (category: any) => {
    ...
    try {
      // Ensure room exists by trying to get messages (creates room implicitly on backend)
