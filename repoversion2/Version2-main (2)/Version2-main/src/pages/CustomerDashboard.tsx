...
export default function CustomerDashboard() {
  ...
  const cmsConfig = getConfig('service-list');
  ...
  const filteredServices = services.filter(s => {
    ...
    return Object.entries(filters).every(([k, v]) => String(s[k]) === String(v));
  });
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'requests'), where('customerId', '==', userId));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setRequests(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    }, (error) => handleFirestoreError(error, OperationType.LIST, 'requests'));

    const sQ = query(collection(db, 'services'));
    ...
    const tQ = query(collection(db, 'transactions'), where('userId', '==', userId));
    const tUnsubscribe = onSnapshot(tQ, (snapshot) => {
      setTransactions(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    }, (error) => handleFirestoreError(error, OperationType.LIST, 'transactions'));

    const tkQ = query(collection(db, 'tickets'), where('creatorId', '==', userId));
    const tkUnsubscribe = onSnapshot(tkQ, (snapshot) => {
      setTickets(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    }, (error) => handleFirestoreError(error, OperationType.LIST, 'tickets'));

    const cQ = query(collection(db, 'contracts'), where('customerId', '==', userId));
    ...
    const kQ = query(collection(db, 'kyc'), where('userId', '==', userId));
  ...
  const handleBecomeProvider = async () => {
    ...
    // Strict check: kycData must exist, be personal type, and status must be 'verified'
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <button 
                      ...
                      className="px-6 py-3 border border-app-border rounded-2xl font-bold text-sm hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-all text-app-text"
                    >
                      View Service
                    </button>
                    <button 
                      onClick={() => navigate(`/chat/${req.id}`)}
                      className="px-6 py-3 bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:scale-105 transition-all shadow-lg shadow-neutral-900/10"
                    >
                      Track & Chat
                    </button>
                  </div>
      ...
      <div className="space-y-12 pb-20">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-neutral-500 font-medium">Welcome back, {auth.currentUser?.displayName?.split(' ')[0]}. Your neighborhood awaits.</p>
          </div>
          <div className="flex items-center gap-3">
            <button 
              onClick={() => navigate('/order-wizard')}
              className="px-6 py-3 bg-emerald-500 text-black rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 hover:scale-105 transition-all shadow-xl shadow-emerald-500/20"
            >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {[
                  ...
                  { label: 'Address Book', icon: MapPin, path: '/profile' },
                  { label: 'Support Center', icon: AlertCircle, path: '/profile' },
                ].map((link) => (
                  <button 
                    key={link.label} 
