      ...
      <table className="min-w-[960px] w-full border-collapse text-left text-sm">
        <thead>
          <tr className="border-b border-app-border bg-neutral-50/80 text-[11px] font-black uppercase tracking-widest text-neutral-500 dark:bg-neutral-900/40 dark:text-neutral-400">
            ...
            <th className="px-4 py-3">Current ver.</th>
        ...
        <tbody>
          {rows.map((r) => {
            ...
            return (
              <tr
                key={r.id}
                className="cursor-pointer border-b border-app-border last:border-0 hover:bg-neutral-50/60 dark:hover:bg-neutral-800/40"
                onClick={() => onOpen(r.id)}
              >
                <td className="px-4 py-3 font-mono text-xs text-app-text">{r.order.id}</td>
                <td className="px-4 py-3 text-app-text">{r.order.matchedWorkspace?.name ?? '—'}</td>
                ...
                <td className="px-4 py-3">
                  <span
                    className={cn(
                      ...
                      st === 'approved' && 'bg-emerald-200 text-emerald-900 dark:bg-emerald-900/40 dark:text-emerald-100',
                      st === 'sent' && 'bg-sky-200 text-sky-900 dark:bg-sky-900/40 dark:text-sky-100',
                      st === 'rejected' && 'bg-red-200 text-red-900 dark:bg-red-900/40 dark:text-red-100',
                      st === 'draft' && 'bg-neutral-200 text-neutral-800 dark:bg-neutral-700 dark:text-neutral-100',
                      !['approved', 'sent', 'rejected', 'draft'].includes(st) &&
                ...
                <td className="px-4 py-3 tabular-nums text-app-text">{money(cv?.amount ?? null, cv?.currency ?? null)}</td>
                <td className="px-4 py-3 text-xs text-neutral-500">{new Date(r.updatedAt).toLocaleString()}</td>
              </tr>
            );
          })}
        </tbody>
